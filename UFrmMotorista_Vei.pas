...
var
  FrmMotorista_Vei: TFrmMotorista_Vei;
...
procedure TFrmMotorista_Vei.Action1Execute(Sender: TObject);
begin
...
procedure TFrmMotorista_Vei.Action2Execute(Sender: TObject);
begin
...
procedure TFrmMotorista_Vei.Action3Execute(Sender: TObject);
 VAR
 I : INTEGER;
begin

{
        if Frm_Motorista<>nil then
                   begin
                ...
                for i := 0 to DBGrid1.SelectedRows.Count - 1 do
                    begin
                        ...
                        dm.query.CLOSE;
                        dm.query.SQL.Text:='delete from MDFE_CADMOTORISTA where id_veiculo =:T and id_motorista =:G';
                        dm.query.Params[0].AsInteger:=dm.CD_VeiculosID.AsInteger;
                        dm.query.Params[1].AsInteger:=Query_MotoristaID_CLIENTE.AsInteger;

                        dm.query.ExecSQL;

                        dm.CD_CADMOTORISTA.Refresh;

                        dm.CD_CADMOTORISTA.Insert;
                        dm.CD_CADMOTORISTAID_VEICULO.AsInteger:=dm.CD_VeiculosID.AsInteger;
                        dm.CD_CADMOTORISTAID_MOTORISTA.AsInteger:= Query_MotoristaID_CLIENTE.AsInteger;
                        dm.CD_CADMOTORISTA.Post;
...
procedure TFrmMotorista_Vei.Action4Execute(Sender: TObject);
begin
close;
dm.CD_CADMOTORISTA.cancel;
end;

procedure TFrmMotorista_Vei.Act_SairExecute(Sender: TObject);
begin
close;
end;

procedure TFrmMotorista_Vei.ComboBox1Select(Sender: TObject);
begin
Edit1.Clear;
...
procedure TFrmMotorista_Vei.DBGrid1CellClick(Column: TColumn);
begin       {
...
procedure TFrmMotorista_Vei.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TFrmMotorista_Vei.Edit1Change(Sender: TObject);
var
b:string;
begin
...
procedure TFrmMotorista_Vei.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
    if ComboBox1.ItemIndex=0 then
       if not (Key in['0'..'9',Chr(8)]) then Key:= #0;
end;

procedure TFrmMotorista_Vei.FormShow(Sender: TObject);
begin
 {     dm.CD_Table_Clientes.close;
