...
type
  TCDM = class(TDataModule)
    ...
    CD_Emissor_CTEFRETE: TFloatField;
    CD_Emissor_CTEDESPACHO: TFloatField;
    CD_Emissor_CTEST: TStringField;
    CD_Emissor_CTETIPOEMI: TIntegerField;
    CD_Emissor_CTECODIGO: TStringField;
    FDConnection1: TFDConnection;
    Query_Unidade: TFDQuery;
    DS_Query_Unidade: TDataSource;
    Query_UnidadeUN: TStringField;
    Query_UnidadeQNT: TBCDField;
    CD_CTE_UNID: TIntegerField;
    CD_N_OperacaoREDUCAO_ICMS_SUB: TSingleField;
    CD_N_OperacaoSUBSTITUICAO: TSingleField;
    CD_N_OperacaoID_EMPRESA: TIntegerField;
    CD_N_OperacaoMOB_ACESSO: TSmallintField;
    CD_N_OperacaoMOB_ORDEM: TSmallintField;
    Query_Grade_Carta: TFDQuery;
    Query_Grade_CartaID: TIntegerField;
    Query_Grade_CartaCHAVE: TStringField;
    Query_Grade_CartaEVENTO: TDateField;
    Query_Grade_CartaCPF_CNPJ: TStringField;
    Query_Grade_CartaSEQUENCIA: TIntegerField;
    CD_CartaCHAVE_CC: TStringField;
    DS_Query_Grade_Carta: TDataSource;
    Query_Grade_CartaCHAVE_CC: TStringField;
    ...
    procedure CD_CTE_UNBeforeInsert(DataSet: TDataSet);
    procedure CD_CTE_UNAfterInsert(DataSet: TDataSet);
  private
...
procedure TCDM.CD_CTE_UNAfterInsert(DataSet: TDataSet);
begin
 CD_CTE_UNID.AsInteger:=IDGEN_ID.AsInteger;
end;

procedure TCDM.CD_CTE_UNBeforeInsert(DataSet: TDataSet);
begin
      id.Close;
      id.SQL.Clear;
      id.SQL.Add('select gen_id(gen_cte_uni_id,1) from rdb$database');
      id.open;
end;
...
begin
     ...
     CD_Emissor_CTEID_TOMADOR.AsInteger:=1;
     ...
     CD_Emissor_CTETIPOEMI.AsInteger:=1;
//     CD_Emissor_CTEREMETENTE.AsInteger:=2;
     ...
     CD_Emissor_CTETIPOEMI.AsInteger:=1;
     CD_Emissor_CTEID_TOMADOR.AsInteger:=5;
...
begin
     CD_N_Operacao.Locate('ID_NATUREZA',CD_Emissor_CTEid_natureza.Asinteger,[loCaseInsensitive]);

     CDM.CD_Emissor_CTECST.AsString:=CDM.CD_N_OperacaoORIGEM_MERCADORIA.AsString+CDM.CD_N_OperacaoTRIBUTACAO.AsString;

     CDM.CD_Emissor_CTECFOP.AsString:=CD_N_OperacaoCODIGO_FISCAL.asstring;
     CDM.CD_Emissor_CTEST.AsString:=CDM.CD_N_OperacaoTRIBUTACAO.AsString;
...
begin
     ...
     CD_N_OperacaoID_EMPRESA.AsInteger:=Frm_Principal.empresa;
...
begin
           FDConnection1.Params.Values['DataBase'] := conexao.Params.Values['DataBase'];
           FDConnection1.Params.Values['user_name'] :=conexao.Params.Values['user_name'];
           FDConnection1.Params.Values['password'] :=conexao.Params.Values['password'];
